...
public class UserController {
    ...
    @ResponseBody
    public Object findEmail(@RequestParam("email") String email) {
        Map<String, Object> resultMap = new HashMap<>();
        User user = userService.findEmail(email);
        if (user == null) {
            resultMap.put("code", "1");
            resultMap.put("message", "email可以使用");
        } else {
            resultMap.put("code", "2");
            resultMap.put("message", "email已经被使用,换一个邮箱吧");
        }
        return JSONObject.toJSONString(resultMap);
    }
    ...
    @ResponseBody
    public Object findQQ(@RequestParam("qq") String qq) {
        Map<String, Object> resultMap = new HashMap<>();
        User user = userService.findQQ(qq);
        if (user == null) {
            resultMap.put("code", "1");
            resultMap.put("message", "QQ可以使用");
        } else {
            resultMap.put("code", "2");
            resultMap.put("message", "QQ已经被使用，起个更创意的名字吧");
        }
        return JSONObject.toJSONString(resultMap);
    }

    @RequestMapping("updateUsernaeOrEmailOrQQ")
    @ResponseBody
    public Object updateUsernaeOrEmailOrQQ(@RequestParam(value = "username", required = false) String username,
        ...
        if (user == null) {
            ...
            if (userService.updateUsernaeOrEmailOrQQ(username, email, qq, file, user.getId()) > 0) {
                Tools.sessionRefresh(currentUser, userService.findtelePhone(user.getTelephone()));
                resultMap.put("code", "1");
